...
// TestNoHistory verifies that a fresh installation with no state files produces
// an overall non-Healthy grade (never a false green).
func TestNoHistory(t *testing.T) {
	...
	if s.Overall == GradeHealthy {
		t.Errorf("overall = %v, want not-Healthy on fresh install", s.Overall)
	}
	for name, sig := range s.Signals {
		if name == SignalDisk {
			continue // disk is always graded from the real host volume
		}
...
func TestAllHealthy(t *testing.T) {
	...
	// Overall must equal the disk grade (or Healthy if disk is fine); all other
	// signals are Healthy so the only source of elevation is disk.
	...
	if s.Overall != wantOverall {
		t.Errorf("overall = %v, want %v; signals: %v", s.Overall, wantOverall, s.Signals)
	}
	for name, sig := range s.Signals {
		if name == SignalDisk {
