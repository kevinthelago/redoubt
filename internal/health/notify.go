...
// Notify evaluates the current health state (using reader, thresholds, and now),
// writes status.json, and fires notifications for any signal that changed grade
// since the last call (with cooldown/dedupe).
func (n *Notifier) Notify(ctx context.Context, current Status) error {
	...
	for signal, ss := range current.Signals {
		...
		// Enforce cooldown: suppress if we notified recently and it's the same
		// direction (getting worse). Always notify on recovery.
		...
		if err := n.send(ctx, signal, ss); err != nil {
			// Non-fatal: log to stderr but keep going.
			fmt.Printf("notify: %v\n", err)
		}
...
// It serializes Grade as its string form.
